...
(function () {
  ...
  const reUnicodeWord = RegExp([`${rsUpper}?${rsLower}+${rsOptContrLower}(?=${[rsBreak, rsUpper, '$'].join('|')})`, `${rsMiscUpper}+${rsOptContrUpper}(?=${[rsBreak, rsUpper + rsMiscLower, '$'].join('|')})`, `${rsUpper}?${rsMiscLower}+${rsOptContrLower}`, `${rsUpper}+${rsOptContrUpper}`, rsOrdUpper, rsOrdLower, rsDigits, rsEmoji].join('|'), 'g');
  ...
  const contextProps = ['Array', 'Buffer', 'DataView', 'Date', 'Error', 'Float32Array', 'Float64Array', 'Function', 'Int8Array', 'Int16Array', 'Int32Array', 'Map', 'Math', 'Object', 'Promise', 'RegExp', 'Set', 'String', 'Symbol', 'TypeError', 'Uint8Array', 'Uint8ClampedArray', 'Uint16Array', 'Uint32Array', 'WeakMap', '_', 'clearTimeout', 'isFinite', 'parseInt', 'setTimeout'];
  ...
  const cloneableTags = {};
  cloneableTags[argsTag] = cloneableTags[arrayTag] = cloneableTags[arrayBufferTag] = cloneableTags[dataViewTag] = cloneableTags[boolTag] = cloneableTags[dateTag] = cloneableTags[float32Tag] = cloneableTags[float64Tag] = cloneableTags[int8Tag] = cloneableTags[int16Tag] = cloneableTags[int32Tag] = cloneableTags[mapTag] = cloneableTags[numberTag] = cloneableTags[objectTag] = cloneableTags[regexpTag] = cloneableTags[setTag] = cloneableTags[stringTag] = cloneableTags[symbolTag] = cloneableTags[
    uint8Tag
  ...
  const runInContext = function runInContext(context) {
    ...
    function createRecurry(func, bitmask, wrapFunc, placeholder, thisArg, partials, holders, argPos, ary, arity) {
      ...
      const newData = [func, bitmask, thisArg, newPartials, newHolders, newPartialsRight, newHoldersRight, argPos, ary, arity];
     ...
     * // => [1]
     */
    const difference = baseRest((array, values) => (isArrayLikeObject(array) ? baseDifference(array, baseFlatten(values, 1, isArrayLikeObject, true)) : []));
     ...
     * // => [3]
     */
    const without = baseRest((array, values) => (isArrayLikeObject(array) ? baseDifference(array, values) : []));
    ...
    function template(string, options, guard) {
      ...
      const reDelimiters = RegExp(`${(options.escape || reNoMatch).source}|${interpolate.source}|${(interpolate === reInterpolate ? reEsTemplate : reNoMatch).source}|${(options.evaluate || reNoMatch).source}|$`, 'g');

      // Use a sourceURL for easier debugging.
      // The sourceURL gets injected into the source that's eval-ed, so be careful
      // with lookup (in case of e.g. prototype pollution), and strip newlines if any.
      // A newline wouldn't be a valid sourceURL anyway, and it'd enable code injection.
      const sourceURL = `//# sourceURL=${hasOwnProperty.call(options, 'sourceURL') ? `${options.sourceURL}`.replace(/[\r\n]/g, ' ') : `lodash.templateSources[${++templateCounter}]`}\n`;
      ...
      const result = attempt(() => Function(importsKeys, `${sourceURL}return ${source}`).apply(undefined, importsValues));
